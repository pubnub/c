 ...
 * compiled-in default.
 */
static pubnub_crypto_provider_t pn_openssl_default = {
    .identifier   = {0, 0, 0, 0},
    .encrypt_size = NULL,
    .encrypt      = NULL,
    .decrypt      = NULL,
    .hmac_sha256  = pn_openssl_hmac_sha256,
    .init         = NULL,
    .deinit       = NULL,
};
...
{
    return &pn_openssl_default;
}

pubnub_crypto_provider_t* pubnub_cryptor_aes_cbc_create(const char* cipher_key,
                                                        pubnub_allocator_provider_t* alloc)
{
    return pn_openssl_cryptor_aes_cbc_create(cipher_key, alloc);
}
...
pubnub_crypto_provider_t* pubnub_cryptor_legacy_create(const char* cipher_key,
                                                       ...
                                                       pubnub_allocator_provider_t* alloc)
{
    return pn_openssl_cryptor_legacy_create(cipher_key, use_random_iv, alloc);
}

void pubnub_cryptor_destroy(pubnub_crypto_provider_t* cryptor)
{
    pn_openssl_cryptor_destroy(cryptor);
}
