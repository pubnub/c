...
#error "app_context_api_member.c requires PUBNUB_ENABLE_APP_CONTEXT=ON"
#endif
...
pubnub_future_t pubnub_get_channel_members(pubnub_context_t* ctx,
                                           const pubnub_get_channel_members_opts_t* opts)
{
    ...
    rc = pn_members_build_path(&prep.entry->http_request,
                               prep.allocator,
...
pubnub_future_t pubnub_set_channel_members(pubnub_context_t* ctx,
...
pubnub_app_context_page_t pubnub_get_channel_members_result(const pubnub_future_t future)
{
...
pubnub_member_t pubnub_get_channel_members_result_member_at(const pubnub_future_t future,
                                                            const size_t index)
{
    pubnub_member_t                result   = {0};
    pn_request_t*                  slot     = NULL;
    ...
    pn_member_parse(parsed->serial, item, &result);
    return result;
}

pubnub_app_context_page_t pubnub_set_channel_members_result(const pubnub_future_t future)
{
    pubnub_app_context_page_t      page = {0};
    pn_request_t*                  slot = NULL;
    const pn_app_context_parsed_t* parsed =
...
pubnub_member_t pubnub_set_channel_members_result_member_at(const pubnub_future_t future,
                                                            const size_t index)
{
    pubnub_member_t                result   = {0};
    pn_request_t*                  slot     = NULL;
