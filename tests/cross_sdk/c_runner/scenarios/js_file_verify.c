...
xs_result_t run_js_file_verify(const xs_context_t* ctx)
{
    ...
    /* Download and verify file content (exact match). */
    ...
    if (0 == content_ok) {
        pn_snprintf(r.detail,
                    sizeof(r.detail),
                    "file content mismatch: expected '%s' (%d bytes)",
                    content,
                    (int)clen);
        pubnub_destroy(pn);
        return r;
     ...
     * JS uploads with message=content so the file notification in the
     * channel history carries that same string in the "message" field. */
    ...
    if (PUBNUB_OK == st) {
        ...
        if (0U < fres.channel_count) {
            ...
            for (i = 0U; i < (size_t)ch.message_count && 0 == notif_ok; ++i) {
                ...
                /* msg.message is the file notification object:
                 * { "message": <attached-text>, "file": { "id": ..., "name": ... } }
                 * Verify "message" field equals the expected content. */
                {
    ...
    if (0 != notif_ok) {
        pn_snprintf(r.detail,
                    sizeof(r.detail),
                    "file content and notification message both match '%s'",
                    content);
    } else {
        pn_snprintf(
            r.detail,
            sizeof(r.detail),
            "file content matches but notification message '%s' not found",
            content);
    }
