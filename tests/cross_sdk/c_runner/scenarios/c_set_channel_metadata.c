...
xs_result_t run_c_set_channel_metadata(const xs_context_t* ctx)
{
    ...
    opts.name    = NULL != ctx->content ? ctx->content : "C-Chan";

    fut = pubnub_set_channel_metadata(pn, &opts);
    st  = pubnub_await(fut);
    if (PUBNUB_OK != st) {
        pubnub_string_view_t em = pubnub_response_error_message(fut);
        pn_snprintf(r.detail,
                    sizeof(r.detail),
                    "set_channel_metadata failed: %s (http=%d %.*s)",
                    pubnub_res_str(st),
    ...
    meta = pubnub_set_channel_metadata_result(fut);
    pn_snprintf(r.detail,
                sizeof(r.detail),
                "set channel=%.*s",
