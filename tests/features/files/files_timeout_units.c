...
static const uint8_t k_ok_body[] = "{\"status\":200,\"data\":[],\"count\":0}";
...
static void list_files_should_use_context_timeout_when_opts_timeout_is_zero(void** state)
{
    (void)state;
    reset_chain();
    ...
    pubnub_future_t fut = pubnub_list_files(ctx,
...
static void
list_files_should_use_per_request_timeout_when_opts_timeout_is_nonzero(void** state)
{
    (void)state;
    reset_chain();
...
#if !PUBNUB_CFG_NO_HEAP
        cmocka_unit_test(
            list_files_should_use_context_timeout_when_opts_timeout_is_zero),
        cmocka_unit_test(
            list_files_should_use_per_request_timeout_when_opts_timeout_is_nonzero),
#else
