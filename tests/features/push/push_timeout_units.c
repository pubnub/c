...
static const uint8_t k_ok_body[] = "[1,\"Modified Channels\"]";
...
static void push_should_use_context_timeout_when_opts_timeout_is_zero(void** state)
{
    (void)state;
    reset_chain();
...
static void push_should_use_per_request_timeout_when_opts_timeout_is_nonzero(void** state)
{
    (void)state;
    reset_chain();
...
#if !PUBNUB_CFG_NO_HEAP
        cmocka_unit_test(push_should_use_context_timeout_when_opts_timeout_is_zero),
        cmocka_unit_test(
            push_should_use_per_request_timeout_when_opts_timeout_is_nonzero),
#else
