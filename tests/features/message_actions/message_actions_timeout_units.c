...
static const uint8_t k_ok_body[] =
    "{\"status\":200,\"data\":{\"type\":\"reaction\",\"value\":\"smiley\","
    "\"uuid\":\"tester\",\"messageTimetoken\":\"17000000000000000\","
    "\"actionTimetoken\":\"17000000000000001\"}}";
...
static void
message_actions_should_use_context_timeout_when_opts_timeout_is_zero(void** state)
{
    (void)state;
    reset_chain();
    ...
    pubnub_future_t fut =
        pubnub_add_message_action(ctx,
                                  &(pubnub_add_message_action_opts_t){
                                      .channel           = "ch",
                                      .message_timetoken = "17000000000000000",
                                      .type              = "reaction",
                                      .value             = "smiley",
                                      .timeout_ms        = 0,
                                  });
...
static void
message_actions_should_use_per_request_timeout_when_opts_timeout_is_nonzero(void** state)
{
    (void)state;
    reset_chain();
...
#if !PUBNUB_CFG_NO_HEAP
        cmocka_unit_test(
            message_actions_should_use_context_timeout_when_opts_timeout_is_zero),
        cmocka_unit_test(
            message_actions_should_use_per_request_timeout_when_opts_timeout_is_nonzero),
#else
